...
export default async function SaludAgenciaPage({
  ...
  const [{ data: settingsRow }, { data: clientsRaw }, { data: svcRaw }, { data: pubsRaw }] =
    await Promise.all([
      ...
      admin
        ...
        .select("cliente_id, tipo, monto_mensual, moneda, activo")
        .eq("activo", true),
      admin
        .from("publications")
  ...
  const rows = clients
    .map((c) => {
      ...
      return { c, pack, reels, posts, mrr, hasPaid, costo, margen, margenPct };
    })
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-xs text-muted-foreground">
        ...
        tiene pauta. No incluye costos generales (oficina, software) ni la comisión del
        closer (one-time). Es el costo directo de entregar cada cuenta.
      </p>
    </div>
  );
}
